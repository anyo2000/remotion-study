...
// 순환계 구간(59.7s~73.0s) 음성 삭제 후 타임스탬프 재계산
const S = [0, 615, 1173, 1791, 2310, 2808, 3231, 3774] as const;
const TOTAL = 4641;

const P = PALETTES.aprilGift;

export const AprilGift2: React.FC = () => {
  return (
    <AbsoluteFill>
      <SpringDecor />
      ...
      <Sequence from={S[3]} durationInFrames={S[4] - S[3]} name="고령생활비">
        <Scene5Senior />
      </Sequence>

      <Sequence from={S[4]} durationInFrames={S[5] - S[4]} name="프리미엄올인원">
        <Scene6Premium />
      </Sequence>

      <Sequence from={S[5]} durationInFrames={S[6] - S[5]} name="최저보험료">
        <Scene7MinPremium />
      </Sequence>

      <Sequence from={S[6]} durationInFrames={S[7] - S[6]} name="0540어린이">
        <Scene8Children />
      </Sequence>

      <Sequence from={S[7]} durationInFrames={TOTAL - S[7]} name="마무리CTA">
        <Scene9CTA />
      </Sequence>

      <Audio src={staticFile("audio/april-gift2-aoede.wav")} />
